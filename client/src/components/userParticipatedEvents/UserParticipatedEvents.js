    ...
    <div className='user-particiaptedd-events'>
        <div>
           ...
           {/* {window.innerWidth <=768 && <div>
                <div>
                    <Select value={defaultValueStatus} options={options} styles={portfolioMobStyles} onChange={(opt) => {setDefaultValueStatus(opt); setDefaultValueMarket({value: 0, label: 'Market'})}}/>
                </div>
                <div>
                    <Select value={defaultValueMarket} options={categoryOption} styles={portfolioMobStyles} onChange={(opt)=> {setDefaultValueMarket(opt); setDefaultValueStatus({value: 0, label: 'Status'})}}/>
                </div>
     ...
     {<div className='participated-event-filteration'>
      ...
      <div className='participated-event'>
            <table responsive={true}>
              ...
              {events?.map((event, index) =>{
                    return <Link to={`/event-detail/${event._id}`} className='event-item-mob'>
