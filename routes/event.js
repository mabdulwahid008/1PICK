...
router.get('/participated', authorization, async (req, res) => {
    ...
    try {
        ...
        participatedEvents = await db.query(`SELECT EVENTS._id, address as creator, creator_id, e_start, image_cid, pick, BETTING.created_on, EVENTS.title, BETTING.bet_amount, BETTING.is_yes, EVENTS.is_active, EVENTS.executed_as FROM BETTING 
                                            INNER JOIN EVENTS
        ...
        createdEvents = await db.query(`SELECT EVENTS._id, address as creator, e_start, creator_id, image_cid, pick, EVENTS.created_on, EVENTS.title, EVENTS.is_active, EVENTS.executed_as FROM EVENTS
                                                INNER JOIN CATEGORIES
                                                    ON CATEGORIES._id = EVENTS.c_id 
                                                INNER JOIN USERS
                                                    ON USERS._id = EVENTS.creator_id
        ...
        favouriteEvents = await db.query(`SELECT EVENTS._id, address as creator, e_start, creator_id, image_cid, pick, FAVOURITE.created_on, EVENTS.title, EVENTS.is_active, EVENTS.executed_as FROM EVENTS
                                                INNER JOIN CATEGORIES
                                                    ON CATEGORIES._id = EVENTS.c_id 
        ...
        return res.status(200).json({portfolio_events:data, action_required_events})
    } catch (error) {
        console.log(error.message);
        return res.status(500).json({ message: error.message })
    }
